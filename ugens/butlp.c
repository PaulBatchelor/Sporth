...
#ifdef DEBUG_MODE
            fprintf(stderr, "butlp: Creating\n")
#endif

            sp_butlp_create(&butlp);
            plumber_add_module(pd, SPORTH_BUTLP, sizeof(sp_butlp), butlp);
            break;
        case PLUMBER_INIT:

#ifdef DEBUG_MODE
            fprintf(stderr, "butlp: Initialising\n")
#endif

            if(sporth_check_args(stack, "ff") != SPORTH_OK) {
                fprintf(stderr,"Not enough arguments for butlp\n");
                stack->error++;
